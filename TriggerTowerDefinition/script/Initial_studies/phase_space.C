...
void phase_space(Long64_t nentries = 1e6, Int_t number_TT=25, Int_t verbose = 2) {
    ...
    TString filename = "/Users/olmo/Dropbox/16_07_FERMILAB_summer_school/CMStrigger/_root/10_SingleMuon_tt25OC_ntuple.root";
    // Open the file
    ...
    TH1D * h_part_phi = new TH1D("h_part_phi","Particles #phi distribution",360,0,2*pigreco);
    ...
    h_part_undet[1] = new TH2D("h_part_undet_vz_eta","Vertex position-#eta distribution of discarded particles",nbins_eta,min_etaTT,max_etaTT,nbins_vz,-max_vz,max_vz);
    ...
    for (Long64_t ievt = 0; ievt < nentries; ++ievt) {
        ...
        if(nstubs==6){
            ...
            for (unsigned istub=0; N_TT_plane_hitted==(Int_t)istub && istub < nstubs; ++istub) {
    ...
    //Saving in files
    TFile *f_out = TFile::Open(Form("_root/10_phase_space.root"),"recreate");
